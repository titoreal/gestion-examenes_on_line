...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($this->is_max_intentos_exceeded($identity))
		{
		...
		$this->increase_intentos($identity);
	 ...
	 * @return boolean
	 */
	public function is_max_intentos_exceeded($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_intentos', 'ion_auth'))
		{
			$max_attempts = $this->config->item('maximum_intentos', 'ion_auth');
			if ($max_attempts > 0)
			{
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_intentos', 'ion_auth'))
		{
			...
			$qres = $this->db->get($this->tables['intentos']);
	...
	public function get_last_attempt_time($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_intentos', 'ion_auth'))
		{
			...
			$qres = $this->db->get($this->tables['intentos'], 1);
	...
	{
		if ($this->config->item('track_intentos', 'ion_auth') && $this->config->item('track_login_ip_address', 'ion_auth'))
		{
	 ...
	 * @return bool
	 */
	public function increase_intentos($identity)
	{
		if ($this->config->item('track_intentos', 'ion_auth'))
		{
			...
			return $this->db->insert($this->tables['intentos'], $data);
		}
		return FALSE;
	}
	 ...
	 *                                                (for security reason, micodigoeum value is lockout_time config value)
	 * @param string|null $ip_address                 IP address
	 *                                                Only used if track_login_ip_address is set to TRUE.
	 *                                                If NULL (default value), the current IP address is used.
	 *                                                Use get_last_attempt_ip($identity) to retrieve a user's last IP
	 ...
	 * @return bool
	 */
	public function clear_intentos($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_intentos', 'ion_auth'))
		{
			...
			return $this->db->delete($this->tables['intentos']);
		}
		return FALSE;
	}
