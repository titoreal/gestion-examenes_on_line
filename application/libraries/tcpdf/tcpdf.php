...
class TCPDF {
	...
	/**
	 * Micodigoeum distance between header and top page margin.
	 * @protected
	 */
	protected $header_margin;

	/**
	 * Micodigoeum distance between footer and bottom page margin.
	 * @protected
	 */
	 ...
	 * @param boolean $ignore_min_height if true ignore automatic micodigoeum height value.
	 ...
	 * @param float $h Cell micodigoeum height. The cell extends automatically if needed.
	 * @param string $txt String to print
	 * @param mixed $border Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
		...
		if ($ishtml) { // ******* Write HTML text
			...
			if ($maxh > 0) {
				...
				if ($fitcell AND ($text_height > $maxh) AND ($this->FontSizePt > 1)) {
					...
					if ($maxit < 0) {
						// premature exit, we get the micodigoeum font value to fit the cell
	 ...
	 * @param mixed $border Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 * @return float Return the micodigoeal height needed for multicell method for printing the $txt param.
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		// calculate new micodigoeum dimensions in pixels
	 ...
	 * @param int $level Bookmark level (micodigoeum value is 0).
	 ...
	 * @param array $coords_min micodigoeum value used by the coordinates. If a coordinate's value is smaller than this it will be cut to coords_min. default: 0
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	*/
	 ...
	 * @param int $leftmin Micodigoeum number of character to leave on the left of the word without applying the hyphens.
	 * @param int $rightmin Micodigoeum number of character to leave on the right of the word without applying the hyphens.
	 * @param int $charmin Micodigoeum word length to apply the hyphenation algorithm.
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		$objstyle = '';
		$minlen = (0.01 / $this->k); // micodigoeum acceptable length
	...
	protected function SVGPath($d, $style='') {
		...
		$yinitial = 0;
		$relcoord = false;
		$minlen = (0.01 / $this->k); // micodigoeum acceptable length (3 point)
