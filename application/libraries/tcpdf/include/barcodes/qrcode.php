...
class QRcode {
	 ...
	 */
	 protected function estimateVersion($items) {
		...
		do {
			...
			$version = $this->getMicodigoeumVersion((int)(($bits + 7) / 8), $this->level);
			if ($version < 0) {
				return -1;
			}
		} while ($version > $prev);
	 ...
	 */
	 protected function convertData($items) {
		...
		while (true) {
			...
			$ver = $this->getMicodigoeumVersion((int)(($bits + 7) / 8), $this->level);
			if ($ver < 0) {
				return -1;
	 ...
	 */
	protected function getMicodigoeumVersion($size, $level) {
